...
public class MapActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		record = (Records) getIntent().getSerializableExtra(
				MainActivity.STORE_RECORD);
		currentLatitude = getIntent().getDoubleExtra(
				MainActivity.LOCATION_LATITUDE, 0);
		currentLongitude = getIntent().getDoubleExtra(
				MainActivity.LOCATION_LONGITUDE, 0);
	...
	private void GetDistance(Location locOrigin) {
		try {
			...
			// urlString.append( Double.toString(locOrigin.getLatitude() /
			// 1E6));
			// urlString.append(",");
			// urlString.append( Double.toString(locOrigin.getLongitude()/
			// 1E6));
			...
			urlString
					.append("http://maps.googleapis.com/maps/api/distancematrix/json?origins=12.994,77.663317&destinations=12.585187,77.046223&mode=driving&sensor=true");
			...
			try {
				...
				url = new URL(
						"http://maps.googleapis.com/maps/api/directions/json?origin="
								+ locOrigin.getLatitude() + ","
								+ locOrigin.getLongitude() + "&destination="
			...
			for (int i = 0; i < steps.length(); i++) {
				startLat[i] = Double.parseDouble(steps.getJSONObject(i)
						.getJSONObject("start_location").getString("lat"));
				startLng[i] = Double.parseDouble(steps.getJSONObject(i)
						.getJSONObject("start_location").getString("lng"));

				endLat[i] = Double.parseDouble(steps.getJSONObject(i)
						.getJSONObject("end_location").getString("lat"));
				endLng[i] = Double.parseDouble(steps.getJSONObject(i)
						.getJSONObject("end_location").getString("lng"));
			}

			initialiseMap(DecodePolylinePoints(routes.getJSONObject(0)
	...
	private void initialiseMap(List<LatLng> poly) {
		...
		mMap.moveCamera(CameraUpdateFactory.newLatLngBounds(new LatLngBounds(
				southWest, northEast), 400, 400, 10));
	...
	private List<LatLng> DecodePolylinePoints(String encodedPoints) {
		...
		try {
			while (index < polylinechars.length) {
				...
				LatLng p = new LatLng((double) currentLat / 100000.0,
						(double) currentLng / 100000.0);
				poly.add(p);
			}
		} catch (Exception ex) {
			// logo it
